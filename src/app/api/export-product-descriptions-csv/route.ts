...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Exporting product descriptions CSV for SKUs:', skus);
    ...
    // 6. Return CSV file
    const filename = `product-descriptions-${new Date().toISOString().split('T')[0]}.csv`;
    
    console.log('Product descriptions CSV export completed successfully');
    ...
    console.error('Product descriptions CSV export failed:', error);
    return NextResponse.json({ 
      success: false, 
...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({
    success: true,
    message: 'Product descriptions CSV export endpoint is ready. Use POST with {"skus": ["SKU1", "SKU2", ...]} to export descriptions as CSV.',
    example: {
      method: 'POST',
      body: {
